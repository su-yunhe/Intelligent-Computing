...
class FullyConnectedLayer(object):
    ...
    def backward(self, top_diff):  # 反向传播的计算
        ...
        self.d_weight = np.matmul(self.input.T, top_diff) # 本层的权重矩阵的梯度
        self.d_bias = top_diff.T
        bottom_diff = np.matmul(top_diff, self.weight.T) # 上一层的输出对损失函数的梯度
        return bottom_diff
    def update_param(self, lr):  # 参数更新
        # TODO：对全连接层参数利用参数进行更新
        self.weight = self.weight - lr * self.d_weight
        self.bias = self.bias - lr * np.sum(self.d_bias, axis=1) # 对每行求和
